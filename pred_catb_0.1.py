...
params = {
    # 'loss_function': 'Tweedie:1ariance_power=1.99',
...
for n in tqdm(range(n_fold)):
    ...
    for i, target in enumerate(target_columns):
        ...
        train_ds = Pool(X_train[if_cols[i]].to_numpy(), label=y_train[target])
        valid_ds = Pool(X_test[if_cols[i]].to_numpy(), label=y_test[target]) 
        cat_model = CatBoostRegressor(**params)
        cat_model.fit(
            train_ds, eval_set=[valid_ds],
            use_best_model=True,
            # metric_period = 100_000,
            early_stopping_rounds= 400,
            verbose_eval =False
                    )
        y_pred_lgb = cat_model.predict(Pool(X_test[if_cols[i]].to_numpy()))
        ...
        best_itr_num_t.append(cat_model.get_best_iteration())
        
        df_train_pred[target].iloc[lv_idx[i][n]] = y_pred_lgb*4
        
        cv_predict_t[target] = cat_model.predict(Pool(test_X[if_cols[i]].to_numpy())) *4
...
for i, c in enumerate(target_columns):
    ...
    if c in [target_columns[n] for n in [0,]]:
        params_base['learning_rate'] = 0.4
        params_base['l2_leaf_reg'] = 2
        # params_base['loss_function'] = 'Tweedie:variance_power=1.99'
...
n_itr_list = [0.03, 0.04, 6] 

# 場所毎の予測を行う
for i, target in enumerate(tqdm(target_columns)):
    X_tmp = X[if_cols[i]]
    y_temp = y.copy()
    train_ds = Pool(X_tmp.to_numpy(), label=y_temp[target])
    ind_params[i]['num_boost_round']=int(best_itr_num_df.iloc[i].max()*n_itr_list[i])
    model = CatBoostRegressor(**ind_params[i])

    #学習
    model.fit(train_ds,verbose_eval =False) 

    #予測
    pred_y = model.predict(Pool(test_X[if_cols[i]].to_numpy()))
...
if submit_cv!=1:
    submit_df.to_csv(f'./sub/cat_{n_fold}cv_{np.mean(valid_score)}_seed_{seed}.csv')
    results4c.to_csv(f'./sub/cat_{n_fold}cv_seed_{seed}_4_en.csv')
