...
for n in tqdm(range(n_fold)):
    ...
    for i, target in enumerate(target_columns):
        ...
        train_ds = xgb.DMatrix(X_train[if_cols[i]], label=y_train[target])
        valid_ds = xgb.DMatrix(X_test[if_cols[i]], label=y_test[target]) 
        # xgb_params = ind_params[i].copy()
        xgb_model = xgb.train(
                xgb_params,train_ds,
                num_boost_round=10_000,
                evals=[(train_ds, 'train'), (valid_ds, 'eval')],
                callbacks=[
                    xgb.callback.EarlyStopping(rounds=400,save_best=True,maximize=False,)
                    ],
                verbose_eval=False,
                              )
        y_pred_lgb = xgb_model.predict(xgb.DMatrix(X_test[if_cols[i]]))
        ...
        best_itr_num_t.append(xgb_model.best_iteration)
        
        df_train_pred[target].iloc[lv_idx[i][n]] = y_pred_lgb*4
        
        cv_predict_t[target] = xgb_model.predict(xgb.DMatrix(test_X[if_cols[i]])) *4
...
n_itr_list = [31, 0.95, 3]

# 場所毎の予測を行う
for i, target in enumerate(tqdm(target_columns)):
    X_tmp = X[if_cols[i]]
    y_temp = y.copy()
    train_ds = xgb.DMatrix(X_tmp, label=y_temp[target])
    #学習
    model = xgb.train(
            ind_params[i],
            train_ds,
    ...
    #予測
    pred_y = model.predict(xgb.DMatrix(test_X[if_cols[i]]))
...
if submit_cv!=1:
    submit_df.to_csv(f'./sub/xgb_{n_fold}cv_{np.mean(valid_score)}_seed_{seed}.csv')
    results4c.to_csv(f'./sub/xgb_{n_fold}cv_seed_{seed}_4_en.csv')
